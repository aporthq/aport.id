...
const APORT_BASE_URL = 'https://api.aport.io';
...
export class APortService {
  ...
  constructor(env: AppEnv) {
    ...
    this.agentPassportBaseUrl = env.AGENT_PASSPORT_BASE_URL || AGENT_PASSPORT_BASE_URL;
    this.apiKey = env.APORT_API_KEY;
    this.orgId = env.APORT_ORG_ID;
    this.providerId = APPROOM_PROVIDER_ID;

    if (!this.apiKey) {
      console.warn('[APort] APORT_API_KEY not set. APort operations will fail.');
    }
    if (!this.orgId) {
      console.warn('[APort] APORT_ORG_ID not set. Organization issuance will fail.');
    }
  }
  ...
  async getCapabilitiesSchema(): Promise<APortResponse<any>> {
    try {
      const response = await fetch(`${this.baseUrl}/api/schema/capabilities-limits`, {
        method: 'GET',
        headers: {
  ...
  async createBuilderPassport(params: {
    ...
    if (!this.orgId) {
      return {
        success: false,
        error: { message: 'APORT_ORG_ID is required for organization issuance' },
      };
    }
    ...
    const passportData: PassportData = {
      name: `${displayName || email}'s Builder Passport`,
      role: 'agent',
      description: `AppRoom builder passport for ${builderId}`,
      regions,
      contact: email,
      assurance: {
        type: assuranceType,
        ...
        proof: {
          verification_id: verificationProof.verification_id || `ver_${builderId}_${Date.now()}`,
          verified_at: verificationProof.verified_at || new Date().toISOString(),
          ...verificationProof,
        },
      },
      capabilities: [
      ...
      // Turn off automatic claim email - we'll send our own
    ...
    try {
      ...
      const claimed = passport.claimed !== false && responseData.claimed !== false;
      ...
      return {
        ...
        data: {
          ...
          ...(responseData.claim_link != null ? { claim_link: String(responseData.claim_link) } : {}),
          ...(responseData.claim_token != null ? { claim_token: String(responseData.claim_token) } : {}),
        },
      };
    } catch (error) {
  ...
  async createAppPassport(params: {
    ...
    regions?: string[];
    metadata?: Record<string, any>;
  }): Promise<APortResponse<{ passportId: string; did: string; assuranceLevel: string }>> {
    const {
      appId,
      builderPassportId,
      builderId,
    ...
    try {
      ...
      const result = (await response.json()) as Record<string, unknown>;
      const passport = (result.data as Record<string, unknown>)?.data ?? (result.data as Record<string, unknown>) ?? {};
   ...
   */
  async getPassport(passportId: string): Promise<APortResponse<any>> {
    if (!passportId) {
      return {
        success: false,
    ...
    try {
      const response = await fetch(`${this.baseUrl}/api/passports/${passportId}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${this.apiKey}`,
  ...
  async updatePassportPolicy(params: {
    ...
    const { passportId, ownerPassportId, capabilities, limits, regions } = params;
    ...
    if (capabilities) {
      updateData.capabilities = capabilities.map((cap) => ({ id: cap, params: {} }));
    }
    ...
    if (Object.keys(updateData).length === 0) {
      return {
        success: false,
        error: { message: 'At least one of capabilities, limits, or regions must be provided' },
      };
    }
    ...
    if (!ownerId) {
      ...
      if (passportResult.success) {
        ownerId = passportResult.data?.owner_id || passportResult.data?.subjectId || this.orgId;
      } else {
        ownerId = this.orgId;
      }
    }
   ...
   */
  async revokePassport(passportId: string, ownerPassportId?: string): Promise<APortResponse<any>> {
    if (!passportId) {
      return {
        success: false,
        error: { message: 'Passport ID is required' },
  ...
  async verifyPolicy(params: {
    ...
    if (!appPassportId && !passport) {
      return {
        success: false,
        error: { message: 'Either app passport ID or passport object is required' },
      };
    }
    ...
    try {
      ...
      const decision = (result.decision ?? data?.decision ?? data) as PolicyDecision | undefined;
  ...
  async verifyFileAccess(params: {
    ...
    const policyPackId = params.operation === 'read' ? 'data.file.read.v1' : 'data.file.write.v1';
